  ...
  <div>
    <div class="news-view">
      <article class="view">
        ...
        <hr>
        <button @click="like" class="btn_like">
          <img class="like" src="/icons/Like.png">
          {{ news.likes }}
        </button>
        <button @click="unLike" class="btn_like">
...
export default {
  ...
  methods: {
    like() {
      ...
      axios
        .post("http://localhost:8080/addLike/" + this.news.newsId, { likes })
        .catch(e => context.error(e));
    },
    unLike() {
      unlikes = ++this.news.unlikes;
      axios
        .post("http://localhost:8080/addUnlike/" + this.news.newsId, {
          unlikes
        })
        .catch(e => context.error(e));
    }
  ...
  asyncData(context) {
    return axios
      .get("http://localhost:8080/getNewsById/" + context.route.params.newsId)
      .then(res => {
        return {
          news: res.data
        };
